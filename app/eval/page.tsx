...
import type { CachedEvalReport, EvalReport } from "@/lib/eval/judge";
...
type CachedRun =
  | { state: "idle" }
  | { state: "running" }
  | { state: "done"; report: CachedEvalReport; elapsedMs: number }
  | { state: "error"; message: string };
...
export default function EvalPage() {
  ...
  const [cachedRuns, setCachedRuns] = useState<Record<string, CachedRun>>({});
  ...
  const runCached = useCallback(async (company: string) => {
    setCachedRuns((r) => ({ ...r, [company]: { state: "running" } }));
    try {
      const res = await fetch("/api/eval", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mode: "cached", company }),
      });
      ...
      setCachedRuns((r) => ({
        ...r,
        [company]: { state: "error", message: (err as Error).message },
      }));
    }
  }, []);
    ...
    <div className="flex flex-1 flex-col">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-8">
            ...
            <div className="mb-3 flex items-baseline justify-between">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Every memo generated by the app. Judged on groundedness +
                  internal consistency (no curated truths).
                </p>
              </div>
              <span className="font-mono text-[11px] text-muted-foreground">
                {cached.length} memo{cached.length === 1 ? "" : "s"}
              </span>
            </div>
            <div className="space-y-3">
              {cached.map((c) => (
                <CachedCard
                  key={c.company}
                  entry={c}
                  state={cachedRuns[c.company] ?? { state: "idle" }}
                  onRun={() => runCached(c.company)}
                />
              ))}
            </div>
          </div>
        ...
        <div className="mt-10 rounded-lg border border-border/70 bg-muted/30 px-5 py-4 text-[13px] leading-relaxed text-muted-foreground">
          ...
          <p className="mt-2">
            <strong>Cached runs</strong> cover every company the app has
            actually memo&rsquo;d. Without curated truths we can&rsquo;t score
            completeness, so the judge grades <strong>groundedness</strong>{" "}
            (verifier pass) plus <strong>internal consistency</strong>{" "}
            (contradictions inside the memo).
          </p>
        </div>
      </main>
    </div>
...
function CachedCard({
  ...
  const generated = new Date(entry.generatedAt).toLocaleDateString(undefined, {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
      ...
      <div className="flex items-center justify-between px-5 py-3.5">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {state.state === "done" ? (
              <Verdict verdict={state.report.verdict} />
            ) : null}
          </div>
          <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 font-mono text-[11px] text-muted-foreground">
            <span>{generated}</span>
            <span>{entry.sources} sources</span>
            {entry.groundedness !== undefined ? (
              <span>Ground {entry.groundedness}</span>
            ) : null}
          ...
          {state.state === "done" ? (
            <div className="flex items-center gap-3 font-mono text-[11px] text-muted-foreground">
              ...
              <span>
                ...
                <span
                  className={
                    state.report.contradictions.length
                      ? "text-destructive"
                      : "text-foreground"
                  }
                >
      ...
      {state.state === "done" ? (
        <>
          ...
          <div className="px-5 py-4">
            ...
            {state.report.contradictions.length ? (
              <ul className="mt-2 space-y-2">
                {state.report.contradictions.map((c, i) => (
                  <li
                    key={i}
                    className="rounded border border-destructive/40 bg-destructive/5 p-2.5 text-[13px]"
                  >
                    <div className="flex items-start gap-2">
                      <ShieldAlert className="mt-0.5 h-3.5 w-3.5 shrink-0 text-destructive" />
                      <div>
                        <div className="font-medium">“{c.claim1}”</div>
